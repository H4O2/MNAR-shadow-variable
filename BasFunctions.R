...
EFFmrf <- function(para, data){
    ...
    IntQ <- function(z, xr, xz, xs, sigmaZ, gam2sigmay2){
        ...
        dnorm(z, meanZ, sigmaZ) * plogis(xs%*%alpha + gamma * (z*beta12 + xr%*%beta1[-1]) - 3/2*gam2sigmay2)
    }
    
    IntPartialQ <- function(z, xr, xz, xs, sigmaZ, gamsigmay2, gam2sigmay2){
        meanZ = xz %*% beta2 - gamma * beta12 * sigmaz2
        logit = plogis(xs%*%alpha + gamma*(z*beta12 + xr%*%beta1[-1]) - 3/2*gam2sigmay2)
        parQ = logit * (1 - logit) * ((z*beta12 + xr%*%beta1[-1]) - 3*gamsigmay2)
        parDens = logit * beta12 * (z - (xz%*%beta2 - gamma*beta12*sigmaz2))
    ...
    Qzfunc <- function(z, xr, xz, xs, sigmaZ, gam2sigmay2){
        meanZ = xz %*% beta2 - gamma * beta12 * sigmaz2
        dnorm(z, mean = meanZ, sd = sigmaZ) * plogis(xs%*%alpha + gamma*(z*beta12 + xr%*%beta1[-1]) - 3/2*gam2sigmay2) * z
    }
